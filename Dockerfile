...
# Use the generic bookworm-slim tag for the runner (always available on Docker Hub)
ARG RUNNER_IMAGE="docker.io/debian:bookworm-slim"

FROM ${BUILDER_IMAGE} AS builder

# install build dependencies (includes git for heroicons dep, and node/npm for assets)
RUN apt-get update \
...
# set build ENV
ENV MIX_ENV="prod"
# Increase timeouts to prevent failures on slow builders
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/coin_tracker ./
